<%@page import="java.util.Date"%>
<%@page import="java.time.LocalDate"%>

<html>
<head>
<title>PET SHOP</title>
	<link rel="stylesheet" href="index.css">
     <link href  = "https://fonts.googleapis.com/icon?family=Material+Icons"  rel = "stylesheet" >
     <script src= 'https://kit.fontawesome.com/a076d05399.js'></script>
     <meta charset = "utf-8">
     <meta name = "viewport" content= "width=device-width , initial-scale=1">
     <link rel = "stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/4.4.1/css/bootstrap.min.css">
     <script src="https://ajax.googleapix.com/ajax/libs/jquery/3.4.1/jquery.min.js"></script>
          <script src="https://cdnjs.cloudflare.com/ajax/libs/popper.js/1.16.0/umd/popper.min.js"></script>
          <script src="https://maxcdn.bootstrapcdn.com/bootstrap/4.4.1/js/bootstrap.min.js"></script>
               <script src="https://ajax.googleapix.com/ajax/libs/jquery/3.4.1/jquery.min.js"></script>
          
     
</head>
<body>
	<jsp:include page="header.jsp"></jsp:include>
	<main class="container-fluid">
		<h3>BreedDogs.</h3>
		
	<section id = "home">
	<div id ="container">
	<div class ="row">
	
	<div class =" col-md-3">
	<h4 class="title" >
	<i class = 'fas fa-bone' style = "font-size: 30px; color:#000;"></i>
	home
	</h4>
	</div>
	<div class = "col-md-3">
	<h4 class = "sub">we offer</h4>
	<h1 class = "con">Proffesional </h1>
	<h4 class = "con2" >daycare,grooming&training </h4>
	
	
	
	</div>
	
	<img class = "img-fluid" src="https://t4.ftcdn.net/jpg/03/07/28/29/240_F_307282946_dbctSHdtrRtWYHoHkUC53JkWkkhm2zJW.jpg">
	</div>
	</div>
	
	</section>	
		
	</main>
</body>
</html>









